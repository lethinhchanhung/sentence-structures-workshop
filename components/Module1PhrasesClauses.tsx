...
import { PHRASES_CLAUSES_TO_ANALYZE } from '../constants';
import { PhraseClauseItem, PhraseClauseType } from '../types';
import { useSound } from '../hooks/useSound';

type PlacedItem = PhraseClauseItem & { status: 'correct' | 'incorrect' };

const DraggableItem: React.FC<{ item: PhraseClauseItem }> = ({ item }) => {
...
const CategoryDropzone: React.FC<{
  category: PhraseClauseType;
  placedItems: PlacedItem[];
  onDropItem: (category: PhraseClauseType, item: PhraseClauseItem) => void;
  ...
  const onDrop = (e: React.DragEvent<HTMLDivElement>) => {
    e.preventDefault();
    setIsOver(false);
    const item = JSON.parse(e.dataTransfer.getData('application/json')) as PhraseClauseItem;
...
const Module1PhrasesClauses: React.FC = () => {
  const [unplacedItems, setUnplacedItems] = useState(PHRASES_CLAUSES_TO_ANALYZE);
  const [placements, setPlacements] = useState<Record<PhraseClauseType, PlacedItem[]>>({
    [PhraseClauseType.Phrase]: [],
    [PhraseClauseType.Clause]: [],
  });
  const { playSound } = useSound();
  
  const handleDropItem = useCallback((category: PhraseClauseType, item: PhraseClauseItem) => {
  ...
  const resetModule = () => {
    setUnplacedItems(PHRASES_CLAUSES_TO_ANALYZE);
    setPlacements({
      [PhraseClauseType.Phrase]: [],
      [PhraseClauseType.Clause]: [],
    });
  ...
  return (
    <div>
       <div className="flex flex-wrap justify-between items-center mb-4 gap-4">
        <div>
          <h3 className="text-xl font-bold text-slate-900">Module 1: Phrases vs. Clauses</h3>
          <p className="text-slate-600">A <span className="font-bold">clause</span> has a subject and a verb. A <span className="font-bold">phrase</span> does not. Drag each item to the correct box.</p>
        </div>
        <button
          onClick={resetModule}
      ...
      <p className="mb-6 font-semibold text-lg text-slate-700">Progress: {correctPlacements} / {PHRASES_CLAUSES_TO_ANALYZE.length} Correct ({totalPlaced} placed)</p>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 bg-slate-100 p-4 rounded-lg border border-slate-200">
          <h4 className="font-bold text-center mb-4 text-slate-700">Items to Analyze</h4>
          {unplacedItems.length > 0 ? (
            ...
            <div className="text-center p-6 bg-slate-200 text-slate-800 rounded-lg border border-slate-300">
              <p className="font-bold text-lg">All Items Categorized!</p>
              <p className="mt-1">You've mastered the first building block!</p>
            </div>
          )}
        </div>
        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
          {Object.values(PhraseClauseType).map(category => (
            <CategoryDropzone 
              key={category}
              category={category}
...
export default Module1PhrasesClauses;
